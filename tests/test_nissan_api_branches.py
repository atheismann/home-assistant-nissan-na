...
MOCK_TOKEN_RESPONSE = {"access_token": "mock_access_token", "refresh_token": "mock_refresh_token"}
MOCK_VEHICLE_LIST = {"vehicles": [{"vin": "VIN123", "model": "Pathfinder", "year": 2022, "nickname": "My Nissan"}]}
MOCK_VEHICLE_STATUS = {
    "batteryLevel": 80,
...
class TestNissanNAApiClientBranches(unittest.TestCase):
    ...
    def test_authenticate_failure(self, mock_post):
        mock_post.return_value = MagicMock(status_code=401, raise_for_status=MagicMock(side_effect=Exception("401 Unauthorized")))
        with self.assertRaises(Exception):
            self.client.authenticate()

    @patch("requests.Session.get")
    def test_get_vehicle_list_empty(self, mock_get):
        mock_get.return_value = MagicMock(status_code=200, json=lambda: {"vehicles": []})
    ...
    def test_lock_doors_error(self, mock_post):
        mock_post.return_value = MagicMock(status_code=500, raise_for_status=MagicMock(side_effect=Exception("500 Error")))
        with self.assertRaises(Exception):
