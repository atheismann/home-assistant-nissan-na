...
class TestAsyncSetupEntry:
    ...
    async def test_setup_entry_creates_trackers_for_vehicles(self):
        ...
        mock_hass = Mock()
        mock_hass.data = {DOMAIN: {"test_entry_id": {"client": Mock()}}}
        ...
        mock_client.get_permissions = AsyncMock(
            return_value=["read_location", "read_battery"]
        )
        mock_client.get_vehicle_status = AsyncMock(
            return_value={"location": {"lat": 37.0, "lon": -122.0}}
        )
...
class TestAsyncAddedToHass:
    ...
    async def test_async_added_to_hass_subscribes_to_webhook(self):
        ...
        with patch(
            "custom_components.nissan_na.device_tracker.async_dispatcher_connect"
        ) as mock_connect:
...
class TestHandleWebhookData:
    ...
    def test_handle_webhook_data_updates_status(self):
        ...
        tracker = NissanVehicleTracker(
            mock_hass, mock_vehicle, initial_status, "test_entry_id"
        )
    ...
    def test_handle_webhook_data_location_change_logged(self):
        ...
        with patch("custom_components.nissan_na.device_tracker._LOGGER") as mock_logger:
            ...
            assert any(
                "location updated" in str(call).lower()
...
class TestAsyncUpdate:
    ...
    async def test_async_update_success(self):
        ...
        mock_client = mock_hass.data[DOMAIN]["test_entry_id"]["client"]
        mock_client.get_vehicle_status = AsyncMock(
            return_value={"location": {"lat": 38.0, "lon": -123.0}, "battery": 85}
        )
