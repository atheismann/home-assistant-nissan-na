...
class TestSensorDefinitions:
    ...
    def test_sensor_definitions_format(self):
        ...
        for definition in SENSOR_DEFINITIONS:
            ...
            signal_id, field_name, name, unit, permission, icon, device_class = (
                definition
            )
...
class TestAsyncSetupEntry:
    ...
    @pytest.mark.asyncio
    async def test_setup_with_vehicles_and_signals(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
        """Test setup creates sensors when signals are available."""
        mock_client.get_vehicle_list = AsyncMock(return_value=[mock_vehicle])
        mock_client.get_vehicle_signals = AsyncMock(
            return_value=["battery.percentRemaining", "charge.state"]
        )
        mock_client.get_vehicle_status = AsyncMock(
            return_value={"battery": {"percentRemaining": 0.85}}
        )
    ...
    @pytest.mark.asyncio
    async def test_setup_without_signals_skips_vehicle(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_setup_with_failed_status_fetch(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
        """Test setup continues when status fetch fails."""
        mock_client.get_vehicle_list = AsyncMock(return_value=[mock_vehicle])
        mock_client.get_vehicle_signals = AsyncMock(
            return_value=["battery.percentRemaining"]
        )
        mock_client.get_vehicle_status = AsyncMock(
    ...
    @pytest.mark.asyncio
    async def test_setup_filters_sensors_by_available_signals(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
        ...
        mock_client.get_vehicle_status = AsyncMock(
            return_value={"battery": {"percentRemaining": 0.85, "range": 250}}
        )
    ...
    @pytest.mark.asyncio
    async def test_setup_only_adds_new_sensors(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
        ...
        unavailable_sensor._signal_id = (
            "fuel.percentRemaining"  # Not in available_signals
        )
        ...
        assert (
            ...
            in mock_hass.data[DOMAIN][mock_config_entry.entry_id]["sensors"][
                mock_vehicle.id
            ]
        )

    @pytest.mark.asyncio
    async def test_rebuild_mode_removes_unavailable_sensors(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
        ...
        with patch.object(entity_registry, "async_get", return_value=mock_registry):
            ...
            await async_setup_entry(
                mock_hass, mock_config_entry, async_add_entities, rebuild_mode=True
            )

            # Should remove unavailable sensor
            assert (
                "fuel.percentRemaining"
                not in mock_hass.data[DOMAIN][mock_config_entry.entry_id]["sensors"][
                    mock_vehicle.id
                ]
            )
    ...
    @pytest.mark.asyncio
    async def test_rebuild_mode_with_no_removals_needed(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_rebuild_mode_with_multiple_vehicles(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
        ...
        mock_client.get_vehicle_list = AsyncMock(
            return_value=[mock_vehicle, mock_vehicle2]
        )
        ...
        with patch.object(entity_registry, "async_get", return_value=mock_registry):
            ...
            sensors_v1 = mock_hass.data[DOMAIN][mock_config_entry.entry_id]["sensors"][
                mock_vehicle.id
            ]
            sensors_v2 = mock_hass.data[DOMAIN][mock_config_entry.entry_id]["sensors"][
                mock_vehicle2.id
            ]
    ...
    @pytest.mark.asyncio
    async def test_rebuild_mode_handles_sensor_without_entity_id(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_boot_preserves_all_existing_sensors(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
        ...
        mock_hass.data = {
            DOMAIN: {
                mock_config_entry.entry_id: {
                    "client": mock_client,
                    "sensors": {mock_vehicle.id: existing_sensors},
                }
            }
        }
        ...
        sensors_dict = mock_hass.data[DOMAIN][mock_config_entry.entry_id]["sensors"][
            mock_vehicle.id
        ]
        ...
        for signal, original in original_sensor_objects.items():
            ...
            assert (
                sensors_dict[signal] is original
            ), f"{signal} sensor object was replaced"
...
class TestNissanGenericSensor:
    ...
    def test_sensor_initialization_with_nickname(
        self, mock_hass, mock_vehicle, mock_config_entry_metric
    ):
    ...
    def test_sensor_initialization_without_nickname(
        self, mock_hass, mock_vehicle_no_nickname, mock_config_entry_metric
    ):
    ...
    def test_sensor_native_value_from_dict(
        self, mock_hass, mock_vehicle, mock_config_entry_metric
    ):
    ...
    def test_sensor_unit_conversion_metric(
        self, mock_hass, mock_vehicle, mock_config_entry_metric
    ):
    ...
    def test_sensor_unit_conversion_imperial(
        self, mock_hass, mock_vehicle, mock_config_entry_imperial
    ):
    ...
    def test_sensor_no_unit_conversion_for_percentage(
        self, mock_hass, mock_vehicle, mock_config_entry_imperial
    ):
    ...
    async def test_sensor_async_added_to_hass(
        ...
        with patch(
            "custom_components.nissan_na.sensor.async_dispatcher_connect"
        ) as mock_connect:
            await sensor.async_added_to_hass()
            mock_connect.assert_called_once()

    @pytest.mark.asyncio
    async def test_sensor_async_will_remove_from_hass(
        self, mock_hass, mock_vehicle, mock_config_entry_metric
    ):
    ...
    def test_sensor_handle_webhook_data(
        self, mock_hass, mock_vehicle, mock_config_entry_metric
    ):
...
class TestFetchStatusBackgroundTask:
    ...
    @pytest.mark.asyncio
    async def test_fetch_status_updates_sensors(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
        ...
        mock_client.get_vehicle_list = AsyncMock(return_value=[mock_vehicle])
        mock_client.get_vehicle_signals = AsyncMock(
            return_value=["battery.percentRemaining", "battery.range"]
        )
    ...
    @pytest.mark.asyncio
    async def test_fetch_status_handles_api_error(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
...
class TestSensorCreationLogic:
    ...
    @pytest.mark.asyncio
    async def test_sensor_creation_filters_by_permissions(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_sensor_creation_avoids_duplicates(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
        ...
        mock_hass.data = {
            DOMAIN: {
                mock_config_entry.entry_id: {
                    "client": mock_client,
                    "sensors": {
                        mock_vehicle.id: {"battery.percentRemaining": existing_sensor}
                    },
                }
            }
        }
        ...
        sensor_entities = [e for e in entities if isinstance(e, NissanGenericSensor)]
        assert (
            len(sensor_entities) == 23
        )  # 2 signals API (one already exists) + 22 permission-based
    ...
    @pytest.mark.asyncio
    async def test_sensor_creation_with_multiple_vehicles(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
...
class TestWebhookSignalSubscription:
    ...
    @pytest.mark.asyncio
    async def test_webhook_signal_subscription_setup(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
        ...
        with patch(
            "custom_components.nissan_na.sensor.async_dispatcher_connect",
            side_effect=mock_dispatcher_connect,
        ):
            await async_setup_entry(mock_hass, mock_config_entry, async_add_entities)
        ...
        webhook_signals = [
            call[0] for call in dispatcher_calls if SIGNAL_WEBHOOK_DATA in call[0]
        ]

        assert len(webhook_signals) > 0

    @pytest.mark.asyncio
    async def test_sensor_webhook_data_merge(
        self, mock_hass, mock_vehicle, mock_config_entry_metric
    ):
    ...
    @pytest.mark.asyncio
    async def test_sensor_webhook_invalid_data_type(
        self, mock_hass, mock_vehicle, mock_config_entry_metric
    ):
    ...
    @pytest.mark.asyncio
    async def test_sensor_async_update(
        self, mock_hass, mock_vehicle, mock_config_entry_metric
    ):
        ...
        mock_hass.data[DOMAIN][mock_config_entry_metric.entry_id] = {
            "client": mock_client
        }
    ...
    @pytest.mark.asyncio
    async def test_sensor_async_update_api_error(
        self, mock_hass, mock_vehicle, mock_config_entry_metric
    ):
...
class TestSensorDefinitionBranches:
    ...
    @pytest.mark.asyncio
    async def test_sensor_with_permission_requirement(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_rebuild_mode_logs_removals(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
        ...
        with patch.object(entity_registry, "async_get", return_value=mock_registry):
            ...
            sensors = mock_hass.data[DOMAIN][mock_config_entry.entry_id]["sensors"][
                mock_vehicle.id
            ]
    ...
    @pytest.mark.asyncio
    async def test_sensor_entity_value_extraction_from_object(
        self, mock_hass, mock_vehicle, mock_config_entry_metric
    ):
    ...
    @pytest.mark.asyncio
    async def test_sensor_value_with_unit_conversion(
        self, mock_hass, mock_vehicle, mock_config_entry_imperial
    ):
    ...
    @pytest.mark.asyncio
    async def test_sensor_should_poll(
        self, mock_hass, mock_vehicle, mock_config_entry_metric
    ):
    ...
    @pytest.mark.asyncio
    async def test_initial_refresh_creates_sensors(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
