...
class TestVehicleModel:
    ...
    def test_vehicle_model_all_fields(self):
        vehicle = Vehicle(
            vin="TEST123", id="vehicle_123", model="Leaf", year=2024, make="Nissan"
        )
        assert vehicle.vin == "TEST123"
...
class TestGetAccessToken:
    ...
    async def test_fetches_new_token_when_expired(self):
        ...
        with patch(
            "custom_components.nissan_na.nissan_api.httpx.AsyncClient",
...
class TestGetVehicleList:
    ...
    async def test_get_vehicle_list_success(self):
        ...
        attrs_response = {
            "data": {
                "attributes": {"make": "NISSAN", "model": "Pathfinder", "year": 2025}
            }
        }
...
class TestGetVehicleInfo:
    ...
    async def test_get_vehicle_info_fetches_via_http(self):
        ...
        mock_httpx.get = AsyncMock(
            side_effect=lambda url, **kw: make_resp(
                vin_resp if "/vin" in url else attrs_resp
            )
        )
...
class TestGetVehicleLocation:
    ...
    async def test_get_vehicle_location_success(self):
        ...
        with patch(
            "custom_components.nissan_na.nissan_api.httpx.AsyncClient",
            return_value=_make_v3_get_mock(body),
        ):
...
class TestLockDoors:
    ...
    async def test_lock_doors_success(self):
        ...
        with patch(
            "custom_components.nissan_na.nissan_api.httpx.AsyncClient",
            return_value=_make_v3_post_mock(body),
        ):
...
class TestGetPermissions:
    ...
    async def test_get_permissions_success(self):
        body = {
            "data": {
                "attributes": {"permissions": ["read_location", "control_security"]}
            }
        }
        with patch(
            "custom_components.nissan_na.nissan_api.httpx.AsyncClient",
