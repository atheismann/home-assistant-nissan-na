...
class TestAsyncSetupEntry:
    ...
    async def test_setup_with_charge_control_permission(
        ...
        mock_hass.data = {
            DOMAIN: {mock_config_entry.entry_id: {"client": mock_client, "numbers": {}}}
        }
    ...
    async def test_setup_without_permission(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
    ...
    async def test_setup_without_signal(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
...
class TestNissanChargeLimitNumber:
    ...
    def test_number_initialization_with_nickname(
        self, mock_hass, mock_vehicle, mock_client
    ):
    ...
    def test_number_initialization_without_nickname(
        self, mock_hass, mock_vehicle_no_nickname, mock_client
    ):
    ...
    async def test_async_added_to_hass(self, mock_hass, mock_vehicle, mock_client):
        ...
        with patch(
            "custom_components.nissan_na.number.async_dispatcher_connect"
        ) as mock_connect:
    ...
    async def test_async_set_native_value_success(
        ...
        with patch(
            "custom_components.nissan_na.number.httpx.AsyncClient",
    ...
    @pytest.mark.asyncio
    async def test_async_set_native_value_clamps_to_range(
        self, mock_hass, mock_vehicle, mock_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_async_set_native_value_exception(
        self, mock_hass, mock_vehicle, mock_client
    ):
...
class TestAsyncSetupEntryErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_setup_handles_signal_fetch_error(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_setup_handles_permission_fetch_error(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_setup_with_multiple_vehicles(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_setup_webhook_subscription(
        self, mock_hass, mock_config_entry, mock_vehicle, mock_client
    ):
        ...
        webhook_signals = [
            call[0] for call in dispatcher_calls if "nissan_na_webhook_data" in call[0]
        ]
...
class TestNumberWebhookHandling:
    ...
    def test_handle_webhook_data_with_partial_charge_info(
        self, mock_hass, mock_vehicle, mock_client
    ):
...
class TestNumberPropertiesAndAttributes:
    ...
    @pytest.mark.asyncio
    async def test_async_set_native_value_with_204_response(
        self, mock_hass, mock_vehicle, mock_client
    ):
