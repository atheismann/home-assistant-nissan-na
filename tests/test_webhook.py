...
def test_hash_challenge():
    ...
    expected = hmac.new(
        token.encode(), challenge.encode(), hashlib.sha256
    ).hexdigest()
...
async def test_webhook_vehicle_state_event(hass):
    ...
    # Mock dispatcher
    with patch("custom_components.nissan_na.webhook.async_dispatcher_send") as mock_dispatcher:
        response = await async_handle_webhook(hass, "test_webhook_id", request)
...
async def test_webhook_register(hass):
    ...
    with patch("custom_components.nissan_na.webhook.webhook.async_register") as mock_register:
...
async def test_webhook_unregister(hass):
    ...
    with patch("custom_components.nissan_na.webhook.webhook.async_unregister") as mock_unregister:
