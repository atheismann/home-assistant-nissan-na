    ...
    for vehicle in vehicles:
        ...
        if vehicle.id not in data["vehicle_signals"]:
            try:
                ...
                _LOGGER.debug(
                    "Fetched signals for switches on vehicle %s: %d codes",
                    vehicle.id,
                    len(signals),
                )
            ...
            _LOGGER.debug(
                "Using cached signals for switches on vehicle %s: %d codes",
                vehicle.id,
                len(signals),
            )
        ...
        if vehicle.id not in data["vehicle_permissions"]:
            ...
            try:
                ...
                _LOGGER.debug(
                    "Fetched permissions for vehicle %s: %s", vehicle.id, permissions
                )
            except Exception:
                permissions = []
                data["vehicle_permissions"][vehicle.id] = permissions
            ...
            _LOGGER.debug(
                "Using cached permissions for vehicle %s: %s", vehicle.id, permissions
            )
        ...
        if (
            "charge.state" in available_signals or not available_signals
