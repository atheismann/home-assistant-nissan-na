from homeassistant.helpers import config_validation as cv
    async def async_step_options(self, user_input=None):
        ...
        data_schema = vol.Schema({
            vol.Optional("update_interval", default=current.get("update_interval", 15)): int,
        })
...
class NissanNAConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            ...
            return self.async_create_entry(title="Nissan NA", data=user_input, options={"update_interval": user_input.get("update_interval", 15)})
